...
# Atomic Skills Above Long ID 2 Training - OpenVLA-OFT LoRA Finetuning on Above Atomic Long ID 2 Dataset (Both Views)
#
...
CPUS_PER_TASK=64
GPUS=8
JOB_NAME="above_atomic_id_2_both_views"
LOG_DIR="$ENDPOINT/pkgs_baselines/openvla-oft/logs"
LOG_FILE="train_above_atomic_id_2_both_views_%j.out"

# Training on the above atomic long ID 2 dataset (Task 2: Complete Kitchen Organization) with both views
sbatch \
  --nodelist=mirage.ib \
  ...
  --wrap="torchrun --standalone --nnodes 1 --nproc-per-node $GPUS vla-scripts/finetune.py \
    ...
    --run_root_dir runs/libero_above_atomic_long_id2_both_view/1.0.0 \
    ...
    --lora_rank 32 \
    --wandb_entity 'yygx' \
    --wandb_project 'openvla-oft-atomic-skills-above-id-2-both-views' \
    --run_id_note 'atomic_skills_above_id_2--8_acts_chunk--continuous_acts--L1_regression--both_views--proprio_state'"
