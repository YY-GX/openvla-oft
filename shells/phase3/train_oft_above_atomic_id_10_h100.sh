...
# Atomic Skills Above Long ID 10 Training - OpenVLA-OFT LoRA Finetuning on Above Atomic Long ID 10 Dataset
#
...
CPUS_PER_TASK=64
GPUS=4
JOB_NAME="above_atomic_id_10"
LOG_DIR="$ENDPOINT/pkgs_baselines/openvla-oft/logs"
LOG_FILE="train_above_atomic_id_10_%j.out"

# Training on the above atomic long ID 10 dataset
sbatch \
  --partition=h100 \
...
torchrun --standalone --nnodes 1 --nproc-per-node $GPUS vla-scripts/finetune.py \
    ...
    --dataset_name libero_above_atomic_long_id10:1.0.1 \
    --run_root_dir runs/libero_above_atomic_long_id10/1.0.4 \
    ...
    --lora_rank 32 \
    --wandb_entity 'yygx' \
    --wandb_project 'openvla-oft-atomic-skills-above-id-10' \
    --run_id_note 'atomic_skills_above_id_10--8_acts_chunk--continuous_acts--L1_regression--wrist_img--proprio_state'"
