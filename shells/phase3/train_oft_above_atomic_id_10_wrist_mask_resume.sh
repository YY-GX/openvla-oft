...
# Atomic Skills Above Long ID 10 Wrist Mask Training - Resume from 40000 checkpoint
#
# Note: This resumes training from the 40000 checkpoint.
# Dataset uses wrist camera view + segmentation mask (2 views).

CPUS_PER_TASK=64
GPUS=4
JOB_NAME="wrist_mask_resume"
LOG_DIR="$ENDPOINT/pkgs_baselines/openvla-oft/logs"
LOG_FILE="train_above_atomic_id_10_wrist_mask_resume_%j.out"
...
torchrun --standalone --nnodes 1 --nproc-per-node $GPUS vla-scripts/finetune.py \
    --resume True \
    --resume_step 40000 \
    --vla_path 'runs/libero_above_atomic_long_id10_wrist_mask/1.0.0/openvla-7b+libero_above_atomic_long_id10_wrist_mask+b8+lr-0.0005+lora-r32+dropout-0.0--image_aug--atomic_skills_above_id_10_wrist_mask--8_acts_chunk--continuous_acts--L1_regression--both_views--proprio_state--40000_chkpt' \
