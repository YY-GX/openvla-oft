...
# Atomic Skills Above Training - OpenVLA-OFT LoRA Finetuning on Above Atomic Dataset
#
...
CPUS_PER_TASK=32
GPUS=4
JOB_NAME="above_atomic_h100"
LOG_DIR="$ENDPOINT/pkgs_baselines/openvla-oft/logs"
LOG_FILE="train_above_atomic_h100_bs16_%j.out"

# Training on the new above atomic dataset
sbatch \
  --partition=h100 \
  ...
  --wrap="torchrun --standalone --nnodes 1 --nproc-per-node $GPUS vla-scripts/finetune.py \
    ...
    --run_root_dir runs/libero_above_atomic_h100_bs16/1.0.0 \
