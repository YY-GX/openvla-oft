...
class PoseFinetuneConfig:
    ...
    splits_folder: str = "splits"                    # Subfolder containing train/val/test annotation files
    dataset_name: str = "pose_dataset"               # Name of fine-tuning dataset
    run_root_dir: Path = Path("runs")                # Path to directory to store logs & checkpoints
    shuffle_buffer_size: int = 100_000               # Dataloader shuffle buffer size (can reduce if OOM errors occur)

    # Algorithm and architecture
    pose_head_type: str = "gmm"                      # Type of pose head: "gmm", "simple", or "hungarian"
    gmm_num_components: int = 6                      # Number of GMM components (increased from 3 to 6)
    ...
    use_pose_normalization: bool = True              # Whether to normalize Euler angles to [-π, π] range
...
def get_run_id(cfg) -> str:
    ...
    if cfg.run_id_override is not None:
        ...
        if cfg.pose_aug:
            run_id += "--pose_aug"
        if not cfg.use_pose_normalization:
            run_id += "--no_pose_norm"
...
def run_forward_pass(
    ...
    try:
        ...
        if pose_augmentation is not None and pose_augmentation.enabled:
            if cfg.pose_head_type == "hungarian":
                # Hungarian poses have shape (batch_size, 6, pose_dim)
                # Apply augmentation to each pose in the batch
                batch_size, num_poses, pose_dim = target_poses.shape
                augmented_poses = []
                for i in range(batch_size):
                    # Apply augmentation to each of the 6 poses
                    demo_poses = []
                    for j in range(num_poses):
                        pose = target_poses[i, j]  # (pose_dim,)
                        augmented_pose = pose_augmentation(pose.unsqueeze(0)).squeeze(0)  # (pose_dim,)
                        demo_poses.append(augmented_pose)
                    augmented_poses.append(torch.stack(demo_poses))  # (6, pose_dim)
                target_poses = torch.stack(augmented_poses)  # (batch_size, 6, pose_dim)
            else:
                # Regular poses have shape (batch_size, pose_dim)
        ...
        with torch.no_grad():
            # Get predictions for metrics - handle DDP wrapper
            if hasattr(vla, 'module'):
                predictions = vla.module.predict_pose(
                    images=images,
                    text=text,
                    text_attention_mask=text_attention_mask,
                    num_samples=1,
            ...
            if cfg.pose_head_type == "gmm":
                # sampled_poses shape: (batch_size, num_samples, num_pose_tokens, num_components, pose_dim)
                # We need to select one sample, one pose token, and one component for metrics
                sampled_poses = predictions['sampled_poses'].squeeze(1)  # Remove num_samples dim: (batch_size, num_pose_tokens, num_components, pose_dim)
                
                # Select the component with highest weight for metrics (use first pose token)
                weights = predictions['weights']  # (batch_size, num_pose_tokens, num_components)
                
                # Use the first pose token for metrics
                ...
                # Handle different pose head types
                if cfg.pose_head_type == "hungarian":
                    # Hungarian head returns (batch_size, num_poses, pose_dim)
                    # Target poses also have shape (batch_size, 6, pose_dim)
                    # For metrics, we'll use the first pose from predictions and first pose from targets
                    if predicted_poses.dim() == 3:
                        # Use the first pose for metrics
                        predicted_poses = predicted_poses[:, 0, :]  # (batch_size, pose_dim)
                    else:
                        # Already in correct shape
                        pass
                    
                    # Also take the first pose from targets for metrics
                    if target_poses.dim() == 3:
                        target_poses = target_poses[:, 0, :]  # (batch_size, pose_dim)
                    else:
                        # Already in correct shape
                        pass
                ...
                # Ensure both tensors have the same batch size
                if predicted_poses.shape[0] != target_poses.shape[0]:
                    if predicted_poses.shape[0] == 1:
                        predicted_poses = predicted_poses.expand(target_poses.shape[0], -1)
                    elif target_poses.shape[0] == 1:
                        target_poses = target_poses.expand(predicted_poses.shape[0], -1)
                    else:
                        raise ValueError(f"Cannot broadcast shapes: {predicted_poses.shape} vs {target_poses.shape}")
...
def finetune_pose(cfg: PoseFinetuneConfig) -> None:
    ...
    if distributed_state.is_main_process and cfg.wandb_entity != "your-wandb-entity":
        try:
            ...
            cfg.wandb_entity = "your-wandb-entity"  # Disable WandB
    ...
    vla = PoseVLM(
        ...
        gmm_entropy_weight=cfg.gmm_entropy_weight,
        gmm_min_epsilon=cfg.gmm_min_epsilon,
        hungarian_weight=cfg.hungarian_weight,
    ...
    # Debug: Check what's actually trainable
    if distributed_state.is_main_process:
        total_params = sum(p.numel() for p in vla.parameters())
        trainable_params = sum(p.numel() for p in vla.parameters() if p.requires_grad)
        lora_params = sum(p.numel() for name, p in vla.named_parameters() if "lora" in name and p.requires_grad)
        pose_params = sum(p.numel() for name, p in vla.named_parameters() if "pose_head" in name and p.requires_grad)

        print(f"Total params: {total_params:,}")
        print(f"Trainable params: {trainable_params:,}")
        print(f"LoRA params: {lora_params:,}")
        print(f"Pose head params: {pose_params:,}")
        print(f"Expected trainable: {lora_params + pose_params:,}")
        
        # Print some example parameter names for debugging
        print("\nExample trainable parameter names:")
        for name, param in vla.named_parameters():
            if param.requires_grad:
                print(f"  {name}")
                if len([n for n, p in vla.named_parameters() if p.requires_grad]) > 10:
    ...
    # Create datasets
    if cfg.pose_head_type == "hungarian":
        from prismatic.vla.datasets.pose_dataset import HungarianPoseDataset
        train_dataset = HungarianPoseDataset(
            ...
            splits_folder=cfg.splits_folder,
            use_pose_normalization=cfg.use_pose_normalization,
        )
        
        if cfg.use_val_set:
            val_dataset = HungarianPoseDataset(
                data_root=str(cfg.data_root_dir),
                split="val",
                num_images_in_input=cfg.num_images_in_input,
