#!/usr/bin/env python3
"""
evaluate_pose_vlm_skill_aware_regu_or_hung.py

Skill-aware evaluation script for PoseVLM that correctly handles both GMM and Hungarian pose heads.
For each overview image, it considers all valid poses from the same skill (language_description) as correct targets.
...
4. Handles both GMM and Hungarian pose heads
5. Uses different dataset configurations for each pose head type

Usage:
    python vla-scripts/evaluate_pose_vlm_skill_aware_regu_or_hung.py \
...
class SkillAwareEvaluatorReguOrHung:
    """
    ...
    Handles both GMM and Hungarian pose heads with different dataset configurations.
    """
    ...
    def __init__(
        ...
        # Detect pose head type and dataset configuration from checkpoint path
        self._detect_config_from_checkpoint()
        
        # Load model and data
        self._load_model()
        self._load_data()
        
    def _detect_config_from_checkpoint(self):
        """Detect pose head type and dataset configuration from checkpoint path."""
        checkpoint_path = self.checkpoint_path.lower()
        
        if "gmm" in checkpoint_path:
            self.pose_head_type = "gmm"
            self.gmm_num_components = 6  # From training script
            self.splits_folder_for_model = "smaller_splits"  # Use smaller_splits for GMM
            print(f"Detected GMM pose head with {self.gmm_num_components} components")
        elif "hungarian" in checkpoint_path:
            self.pose_head_type = "hungarian"
            self.splits_folder_for_model = "splits"  # Use splits for Hungarian
            print(f"Detected Hungarian pose head")
        else:
            # Default to GMM if can't detect
            self.pose_head_type = "gmm"
            self.gmm_num_components = 6
            self.splits_folder_for_model = "smaller_splits"
            print(f"Could not detect pose head type, defaulting to GMM")
        
        print(f"Using splits folder: {self.splits_folder_for_model}")
    ...
    def _load_model(self):
        ...
        if self.config is None:
            # Fallback: create a simple config based on detected pose head type
            class SimpleConfig:
            ...
            self.config = SimpleConfig(
                pose_head_type=self.pose_head_type,
                gmm_num_components=getattr(self, 'gmm_num_components', 6),
                pose_dim=6,
                num_pose_tokens=6,
        ...
        if self.pose_head_type == "hungarian":
            self.model = PoseVLM(
                ...
                hungarian_weight=0.1,  # From training script
    ...
    def _load_data(self):
        ...
        # Create datasets with appropriate splits folder
        self.train_dataset = create_pose_dataset(
            ...
            splits_folder=self.splits_folder_for_model,
        )
        ...
        # For Hungarian pose head, we need to filter skills from smaller_splits
        if self.pose_head_type == "hungarian":
            self._filter_hungarian_skills()
    ...
    def _filter_hungarian_skills(self):
        """For Hungarian pose head, filter to only include skills present in smaller_splits."""
        if self.pose_head_type != "hungarian":
            return
            
        print("Filtering Hungarian dataset to skills present in smaller_splits...")
        
        # Load smaller_splits annotation to get valid skills
        smaller_splits_annotation_path = os.path.join(self.data_root, "smaller_splits", "train_annotation.csv")
        if os.path.exists(smaller_splits_annotation_path):
            smaller_splits_df = pd.read_csv(smaller_splits_annotation_path)
            valid_skills = set(smaller_splits_df['language_description'].unique())
            print(f"Found {len(valid_skills)} valid skills in smaller_splits: {valid_skills}")
        else:
            print("Warning: smaller_splits annotation not found, using all skills")
            valid_skills = None
        
        # Load current splits annotation
        current_annotation_path = os.path.join(self.data_root, self.splits_folder_for_model, "train_annotation.csv")
        if os.path.exists(current_annotation_path):
            current_df = pd.read_csv(current_annotation_path)
            if valid_skills is not None:
                # Filter to only valid skills
                filtered_df = current_df[current_df['language_description'].isin(valid_skills)]
                print(f"Filtered from {len(current_df)} to {len(filtered_df)} samples")
                
                # Save filtered annotation temporarily
                filtered_annotation_path = os.path.join(self.data_root, self.splits_folder_for_model, "train_annotation_filtered.csv")
                filtered_df.to_csv(filtered_annotation_path, index=False)
                
                # Update dataset to use filtered annotation
                # Note: This is a hack - ideally we'd modify the dataset creation
                # For now, we'll handle this in the evaluation logic
                self.valid_skills = valid_skills
            else:
                self.valid_skills = None
        else:
            print("Warning: Current splits annotation not found")
            self.valid_skills = None
    ...
    def _load_annotation_data(self, split: str) -> Tuple[pd.DataFrame, np.ndarray]:
        ...
        if not os.path.exists(annotation_path):
            ...
            annotation_path = os.path.join(self.data_root, self.splits_folder_for_model, f"{split}_annotation.csv")
            if not os.path.exists(annotation_path):
                raise FileNotFoundError(f"Annotation file not found in {self.data_root} or {self.data_root}/{self.splits_folder_for_model}")
        annotation_df = pd.read_csv(annotation_path)
        
        # For Hungarian pose head, filter to valid skills
        if self.pose_head_type == "hungarian" and hasattr(self, 'valid_skills') and self.valid_skills is not None:
            annotation_df = annotation_df[annotation_df['language_description'].isin(self.valid_skills)]
            print(f"Filtered {split} annotation to {len(annotation_df)} samples with valid skills")
    ...
    def _run_forward_pass(self, batch: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        # For Hungarian pose head, we need to handle the target pose shape differently
        if self.config.pose_head_type == "hungarian":
            # Hungarian pose head expects (batch_size, 6, pose_dim) but dataset provides (batch_size, pose_dim)
            # We'll create a dummy target by repeating the single pose 6 times
            batch_size = target_poses.shape[0]
            pose_dim = target_poses.shape[1] if target_poses.dim() > 1 else target_poses.shape[0]
            
            if target_poses.dim() == 1:
                target_poses = target_poses.unsqueeze(0)  # (1, pose_dim)
            
            # Repeat the target pose 6 times to match Hungarian expectations
            target_poses_hungarian = target_poses.unsqueeze(1).expand(batch_size, 6, pose_dim)
            
            # Compute loss using PoseVLM's built-in method with modified targets
            loss = self.model.compute_loss(
                images=images,
                text=text,
                text_attention_mask=text_attention_mask,
                target_poses=target_poses_hungarian,
            )
        else:
            # GMM pose head - use original targets
            loss = self.model.compute_loss(
                images=images,
                text=text,
                text_attention_mask=text_attention_mask,
        ...
        with torch.no_grad():
            ...
            if self.config.pose_head_type == "gmm":
                ...
                # Hungarian pose head
                predicted_poses = predictions['predicted_poses']  # (batch_size, 6, pose_dim)
                # For evaluation, use the first predicted pose
                selected_poses = predicted_poses[:, 0, :]  # (batch_size, pose_dim)
                
                # Use original target_poses (not the expanded Hungarian version) for metrics
                original_target_poses = batch["pose_targets"].to(self.device).to(torch.bfloat16)
                if original_target_poses.dim() == 1:
                    original_target_poses = original_target_poses.unsqueeze(0)
                
                l1_error = torch.mean(torch.abs(selected_poses - original_target_poses))
                metrics = {
                    "loss": loss.item(),
                    "l1_error": l1_error.item(),
                }
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(dataloader, desc=f"Evaluating {split} ({run_name})")):
                ...
                if self.config.pose_head_type == "gmm":
                    ...
                    predicted_poses = predictions['predicted_poses']  # (batch_size, 6, pose_dim)
                    # For skill-aware evaluation, use the first predicted pose
                    predicted_poses = predicted_poses[:, 0, :]  # (batch_size, pose_dim)
...
def main():
    parser = argparse.ArgumentParser(description="Skill-aware evaluation of PoseVLM (GMM and Hungarian)")
    parser.add_argument(
        "--checkpoint_path",
        type=str,
        required=True,
    ...
    # Create evaluator
    evaluator = SkillAwareEvaluatorReguOrHung(
    ...
    output_path = os.path.join(checkpoint_dir, f"evaluation_results_skill_wise_regu_or_hung_{timestamp}.json")
