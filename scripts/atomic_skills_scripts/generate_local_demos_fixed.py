...
def process_atomic_skill(mapping: Dict, args) -> Tuple[bool, str]:
    ...
    try:
        ...
        for demo_idx, demo_key in enumerate(demo_iterator):
            ...
            if args.debug_single_skill:
                print(f"🔍 Step 2: Task completion detection...")
            temp_env, _ = get_libero_env(task, model_family="openvla", resolution=256)
            temp_obs = temp_env.reset()
            actual_completion_step = None
            for temp_t in range(len(actions)):
            ...
            if args.debug_single_skill:
                if actual_completion_step is not None:
                    print(f"✅ Task completion found at timestep {actual_completion_step}")
                    demo_detail['task_completion_found'] = True
                    demo_detail['task_completion_step'] = actual_completion_step
                else:
                    print(f"❌ Task completion NOT found - demo may be incomplete")
