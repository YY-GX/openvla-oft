...
def process_atomic_skill(mapping: Dict, args) -> Tuple[bool, str]:
    ...
    try:
        ...
        # Debug single skill mode: track detailed statistics
        if args.debug_single_skill:
            debug_stats = {
                'total_demos': len(demo_keys),
                'failed_trigger_detection': 0,
                'failed_task_completion': 0,
                'failed_environment_reset': 0,
                'successful_demos': 0,
                'demo_details': []
            }
            print(f"🔍 DEBUG MODE: Will analyze all {len(demo_keys)} demos in detail")
        ...
        # Process each demonstration
        demo_iterator = tqdm(demo_keys, desc=f"Processing {skill_name} demos") if not args.debug_single_skill else demo_keys
        for demo_idx, demo_key in enumerate(demo_iterator):
            ...
            if args.debug_single_skill:
                print(f"\n{'='*60}")
                print(f"🔍 DEMO {demo_idx+1}/{len(demo_keys)}: {demo_key}")
                print(f"{'='*60}")
                ...
                demo_detail = {
                    ...
                    'trigger_found': False,
                    'trigger_timestep': None,
                    'task_completion_found': False,
            ...
            if args.debug_single_skill:
                print(f"🔍 Step 1: Contact trigger detection...")
            ...
            if trigger_timestep is None:
                if args.debug_single_skill:
                    ...
                    debug_stats['failed_trigger_detection'] += 1
                    demo_detail['failure_reason'] = 'No contact trigger found'
                    debug_stats['demo_details'].append(demo_detail)
                else:
                    print(f"FAILURE: No contact trigger found for {demo_key}")
            ...
            if args.debug_single_skill:
                ...
                demo_detail['trigger_found'] = True
                demo_detail['trigger_timestep'] = trigger_timestep
            
            # Calculate collection bounds for atomic skills
            start_idx = max(0, trigger_timestep - args.atomic_offset)
            
            # SINGLE REPLAY: Complete trajectory replay with full data collection
            if args.debug_single_skill:
                print(f"🔍 Step 2: Single complete replay (no double replay)")
            
            # Reset environment and replay ONCE to collect all trajectory data
            obs = env.reset()
            full_trajectory = []
            actual_completion_step = None
            replay_success = False
            ...
            # SINGLE COMPLETE REPLAY: Replay entire trajectory and collect all data
            for t in range(len(actions)):
                action = actions[t]
                obs, reward, done, _ = env.step(action)
                ...
                if done:
                    ...
                    if args.debug_single_skill:
                        print(f"✅ Task completion found at timestep {t} during replay")
                        demo_detail['task_completion_found'] = True
                        demo_detail['task_completion_step'] = t
                    break
            
            # POST-PROCESS: Slice trajectory based on trigger and completion
            collected_steps = []
            if replay_success:
                # Calculate optimal start_idx
                if actual_completion_step is not None:
                ...
                if args.debug_single_skill:
                    print(f"✅ Sliced trajectory: steps {start_idx} to {end_idx-1} ({len(collected_steps)} steps)")
            else:
                if args.debug_single_skill:
                    print(f"❌ Task completion NOT found during replay")
                    debug_stats['failed_task_completion'] += 1
                    demo_detail['failure_reason'] = 'Task completion not found'
...
def process_pick_skill(mapping: Dict, args) -> Tuple[bool, str]:
    ...
    try:
        ...
        # Debug single skill mode: Print comprehensive summary
        if args.debug_single_skill:
            print(f"\n{'='*80}")
            print(f"🔍 DEBUG SUMMARY FOR SKILL: {skill_name}")
            print(f"{'='*80}")
            print(f"📊 OVERALL STATISTICS:")
            print(f"  Total demos processed: {debug_stats['total_demos']}")
            print(f"  Failed trigger detection: {debug_stats['failed_trigger_detection']}")
            print(f"  Failed task completion: {debug_stats['failed_task_completion']}")
            print(f"  Successful demos: {debug_stats['successful_demos']}")
            print(f"  Success rate: {debug_stats['successful_demos']}/{debug_stats['total_demos']} = {debug_stats['successful_demos']/debug_stats['total_demos']:.1%}")
            
            print(f"\n📋 DETAILED BREAKDOWN:")
            success_demos = []
            failed_trigger_demos = []
            failed_completion_demos = []
            other_failed_demos = []
            
            for detail in debug_stats['demo_details']:
                if detail['final_success']:
                    success_demos.append(detail)
                elif detail['failure_reason'] == 'No contact trigger found':
                    failed_trigger_demos.append(detail)
                elif detail['failure_reason'] == 'Task completion not found':
                    failed_completion_demos.append(detail)
                else:
                    other_failed_demos.append(detail)
            
            print(f"\n✅ SUCCESSFUL DEMOS ({len(success_demos)}):")
            for detail in success_demos:
                print(f"  - {detail['demo_key']}: trigger@{detail['trigger_timestep']}, completion@{detail['task_completion_step']}")
            
            print(f"\n❌ FAILED TRIGGER DETECTION ({len(failed_trigger_demos)}):")
            for detail in failed_trigger_demos:
                print(f"  - {detail['demo_key']}: {detail['demo_length']} steps, no contact detected")
            
            print(f"\n❌ FAILED TASK COMPLETION ({len(failed_completion_demos)}):")
            for detail in failed_completion_demos:
                print(f"  - {detail['demo_key']}: trigger@{detail['trigger_timestep']}, but no task completion")
            
            if other_failed_demos:
                print(f"\n❌ OTHER FAILURES ({len(other_failed_demos)}):")
                for detail in other_failed_demos:
                    print(f"  - {detail['demo_key']}: {detail['failure_reason']}")
            
            print(f"\n{'='*80}")
            
            # Save debug results to file
            debug_file = os.path.join(args.output_dir, f"debug_{skill_name}.json")
            import json
...
Examples:
  ...
  python generate_local_demos_fixed.py --debug
  
  # Debug single skill with detailed analysis (focus on 'open_the_top_drawer_of_the_cabinet')
  python generate_local_demos_fixed.py --debug-single-skill --output_dir debug_single_skill_output
  
  # Full run with custom paths
  python generate_local_demos_fixed.py --atomic_skills_dir /path/to/skills --raw_demo_dir /path/to/demos
        """
    )
    ...
    parser.add_argument(
        "--debug-single-skill",
        action="store_true",
        help="Debug mode: only process 'open_the_top_drawer_of_the_cabinet' skill with detailed analysis"
    )
    ...
    # Process pick skills
    if pick_mappings:
        # Filter for debug-single-skill mode
        if args.debug_single_skill:
            target_skill = "open_the_top_drawer_of_the_cabinet"
            pick_mappings = [m for m in pick_mappings if target_skill in m['skill_name'].lower()]
            print(f"\n🔍 DEBUG SINGLE SKILL MODE: Processing only '{target_skill}' pick skills")
            print(f"Found {len(pick_mappings)} matching pick skills:")
            for m in pick_mappings:
    ...
    if place_mappings:
        ...
        if args.debug_single_skill:
            target_skill = "open_the_top_drawer_of_the_cabinet"
            place_mappings = [m for m in place_mappings if target_skill in m['skill_name'].lower()]
            print(f"\n🔍 DEBUG SINGLE SKILL MODE: Processing only '{target_skill}' place skills")
            print(f"Found {len(place_mappings)} matching place skills:")
    ...
    if atomic_mappings:
        ...
        if args.debug_single_skill:
            target_skill = "open_the_top_drawer_of_the_cabinet"
            atomic_mappings = [m for m in atomic_mappings if target_skill in m['skill_name'].lower()]
            print(f"\n🔍 DEBUG SINGLE SKILL MODE: Processing only '{target_skill}' skills")
            print(f"Found {len(atomic_mappings)} matching skills:")
