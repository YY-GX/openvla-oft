...
for i in $(seq 1 ${NUM_PROCESSES}); do
    ...
    tmux send-keys -t "${SESSION_NAME}" "CUDA_VISIBLE_DEVICES=7 python3 ${PYTHON_SCRIPT} --num_augmentations ${NUM_AUGMENTATIONS} --random_seed ${SEED} --output_path \"${OUTPUT_PATH}\" --resume ${VIDEO_FLAG} --above_height 0.06" C-m
