#!/bin/bash
# Launch parallel processes for demo generation (multi-BDDL skills only)
# Only processes skills with >3 BDDL files to address heterogeneous demos issue
# Each process uses different random seeds and output folders
...
for i in $(seq 1 ${NUM_PROCESSES}); do
    ...
    tmux send-keys -t "${SESSION_NAME}" "CUDA_VISIBLE_DEVICES=7 python3 ${PYTHON_SCRIPT} --multi_bddl_only --num_augmentations ${NUM_AUGMENTATIONS} --random_seed ${SEED} --output_path \"${OUTPUT_PATH}\" --resume ${VIDEO_FLAG}" C-m
