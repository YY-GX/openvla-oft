#!/usr/bin/env python3
"""
Simplified object-EE pose pairs extraction script.

This script uses a much simpler approach:
1. Load .init file (contains states where EE is positioned to interact with target object)
2. Set simulation state  
3. Extract EE pose from simulation
...
No complex contact detection needed - the target object is known from skill mapping.
"""
    ...
    try:
        ...
        for state_idx, initial_state in enumerate(initial_states):
            ...
            try:
                ...
                possible_ee_names = ["robot0_eef", "gripper0_eef", "gripper_eef", "eef"]
                ...
                ee_quat = data.body_xquat[ee_body_id].copy()  # [w, x, y, z] format in MuJoCo
...
def main():
    ...
    output_dir = "/mnt/arc/yygx/pkgs_baselines/openvla-oft/datasets/hdf5_datasets/atomic_local_demos/ee_target_obj_pairs/simple_extraction"
